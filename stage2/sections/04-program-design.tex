...
\paragraph{Approach}
The approach is a pipeline with multiple stages. The first set of stages discovers what the
corpus is about. Each document is embedded into the space introduced in
Section~\ref{sec:computational-ir}, under the geometric assumption that documents with similar
meaning receive nearby vectors. BERTopic \parencite{Grootendorst_BERTopic} then reduces the
dimension of these vectors and clusters them, and each cluster is a topic. The output is the set
of topics $K$ with cluster centroids $\{\boldsymbol{\mu}_k\}_{k \in K}$, where each
$\boldsymbol{\mu}_k \in \mathbb{R}^{m}$ is the average embedding of the documents in topic $k$.
The second set of stages selects, for each document and topic, the passages relevant to that
topic. Each document $d$ is split into passages $P_d = \{p_1, \dots, p_{n_d}\}$, and each passage
$p$ is embedded as $\mathbf{e}_p \in \mathbb{R}^{m}$. Relevance is defined quantitatively:
...
$\text{country} \times \text{topic}$ matrix, with $C$ countries and $|K|$ topics; a dot means the
index runs over all its values (full notation in Appendix~C).
...
$k$, three hypotheses are written: a supporting pole $h_k^{+}$, a neutral one $h_k^{0}$, and an
opposing pole $h_k^{-}$. Each is paired with the extracted text as premise and scored by the NLI
...
and $\alpha, \beta, \gamma \ge 0$ are trade-off weights. $R_i$ is measured relative to the status
quo and normalised to zero. Every agent holds a genuine veto, and consensus emerges from the
trade-off between blocking a proposal and bearing the cost of objecting.
