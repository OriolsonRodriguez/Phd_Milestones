...
\paragraph{Approach}
The approach is a pipeline with multiple stages (full details and notation in \hyperref[app:e]{Appendix~E}). The
...
convention (\hyperref[app:e]{Appendix~E}). It is a convex combination of five fixed class values, and therefore a
...
full credit and fractional credit, each answering a different question (\hyperref[app:e]{Appendix~E}). For a fixed
year, the result is a signed country-by-topic matrix (full notation in \hyperref[app:c]{Appendix~C}).
...
neutrality (examples in \hyperref[app:e]{Appendix~E}). Stance is therefore recovered as a contrast. For each topic,
three hypotheses are written: a supporting pole, a neutral one, and an opposing pole (notation in
\hyperref[app:e]{Appendix~E}). Each is paired with the extracted text as premise and scored by the NLI
...
canary uses the minerals negotiations of the late 1970s and 1980s as documented ground
truth \parencite{jackson2021antarctica} and compares them to recovered countries preferences extracted from the text.
 Finally, a co-sponsorship check tests the assumption that countries co-sponsor a WP because they
...
The tensor pair $(\mathbf{S}, \mathbf{O})$ is tested for predictive content against observed outcomes (see \hyperref[app:f]{Appendix~F}) and the compilation
...
\paragraph{Approach}
A custom negotiation environment is specified in which proposals are presented one at a time. Each proposal is an input text that is mostly about topic. Then the agents (one per consultative party)
...
relaxes the adoption rule (Equation~\ref{eq:adopt}, \hyperref[app:e]{Appendix~E}). This counterfactual is intended to predict the behaviour of the system given a change in the
...
The project uses openly published data from the ATS document database. The project uses open-source software and/or QUT-licensed software. 
