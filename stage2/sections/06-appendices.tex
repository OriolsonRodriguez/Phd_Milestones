...
[Insert RIO completion certificate --- mandatory for Stage 2 approval.]

\section*{Appendix B: Coursework}
\addcontentsline{toc}{section}{Appendix B: Coursework}

[Insert Advanced Information Research Skills (AIRS / IFN006) status and any other
coursework.]
...
$\tilde\pi_{d,k}$ & three-pole distribution (Appendix~C, stance classifier) \\
...
A fixed-year slice of $S$ is a signed country-by-topic matrix. Entries lie in
$[-1,1]$ after aggregation and normalisation: positive values indicate support for
the permissive pole of the topic's axis, negative values support for the restrictive
pole, and values near zero either neutrality or absence of position-taking. Sparsity
is expected; absence of evidence is recorded as missing, not as zero.

\paragraph{Stance classifier (Chapter~1).}
For topic $k$, let $h_k^{+}, h_k^{0}, h_k^{-}$ be the for-, neutral-, and
against-pole hypotheses. For a pooled passage set $d$ and hypothesis $h$, the
debiased evidence is the null-premise-corrected log-ratio
\[
  \lambda(d,h) = \operatorname{clip}\!\Big(
     \log P(\text{entail}\mid d,h) - \log P(\text{entail}\mid \varnothing,h),
     \ \pm 5 \Big),
\]
which removes each hypothesis's marginal acceptability (a directed
pointwise-mutual-information / DCPMI debiasing). The three-pole distribution
...
\[
  ...
  \end{pmatrix},
\]
which is expanded to the five ordinal classes $\pi_{d,k}$ and reduced to the
scalar stance by the expected value of Equation~(1).

\section*{Appendix D: Acknowledgement of AI Use}
\addcontentsline{toc}{section}{Appendix D: Acknowledgement of AI Use}

[Per QUT GRC \S4.4: discuss scope with the principal supervisor before use, and
provide a written acknowledgement of any AI or AI-assisted technology used in
preparing this document, including the tool, its purpose, and the extent of use.]
