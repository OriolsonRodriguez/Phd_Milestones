...
\begin{tikzpicture}[
  ...
  \node[proc, below=of start]       (extract) {Extract veto statements\\ $\to$ (country, target) tuples\\ + rationale \& source quote};
  \node[proc, below=of extract]     (map)     {Map targets to Chapter~1 issues $K$};

  % ── parallel split (primary / secondary) ─────────────────────
  \coordinate (fork) at ($(map.south)+(0,-7mm)$);
  \node[proc] (primary)   at ($(map.south)+(-32mm,-20mm)$) {\textbf{Primary:} opposition tensor $O$\\ (country $\times$ issue $\times$ year)};
  ...
  \node[note, right=of val]     (n4) {Independent research question; risk: exogenous geopolitics not captured in institutional text.};
